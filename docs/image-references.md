...
The reference contract is implemented and covered by provider-free tests. The
source-generated and signed `Hollis Image - Reference Input v2` Shortcut was
imported on an already authorized macOS 27.0 build (`26A5425a`). Qualification
produced 52 valid images across diagnostic and acceptance runs, serialized
with at least three seconds between generations. These are transport successes,
not 52 perfect visual results: earlier prompt formats sometimes ignored scene
changes or lost subjects.

The final renderer puts the newest revision first and retains earlier subject
and text context. All nine final images passed visual review across three-turn
interactive chat, Chat Completions, and Responses sequences: the intended
subjects remained and each requested scene change appeared. All five working
styles were also exercised with and without references, including PNG/JPEG
inputs and local crop/pad processing. Exact identity and pixel editing are not
guaranteed; these tests do not isolate whether the backend conditioned on the
reference pixels rather than the accompanying text.

First-ever permissions on a new Mac remain untested. The ChatGPT Image
Playground Shortcut route has a confirmed host/build-scoped pre-inference
failure; see the qualification note below. This is an experimental local
candidate, not a published release or broad reliability benchmark.
...
the [image-generation setup guide](image-generation.md#setup-generated-parameterized-shortcut).
